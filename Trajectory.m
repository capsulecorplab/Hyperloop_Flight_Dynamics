...
for deltat_LIM = deltat_LIM_max:-0.5:0    % Loop through deltat_LIM, such that desired sim constraints are satisfied
...
%     Fdrag_brake(n) = Fbrakedrag(xdot(n-1),brakegap(n-1))/ (1 - eta);
    ...
    Fdrag = Fdrag_aero(n) + Fdrag_hover(n) + Fdrag_brake(n);
    Fdrag = Fdrag - Flimprop(n);     % Auxiliary drag using LIM
...
%             Fdrag_brakeDecel(i) = Fbrakedrag(xdotDecel(i-1),bDecel(i-1)) / (1 - eta);
            ...
            %% Engage aux propulsion using LIM Phase
            if tDecel(i-1) < (t2 + deltat_LIM)     % Cruising phase constrained by time
                FlimpropDecel(i) = Flimthrust(xdotDecel(i-1));     % Auxiliary thrust using LIM

                % Compute total drag
                Fdrag = Fdrag_aeroDecel(i) + Fdrag_hoverDecel(i) + Fdrag_brakeDecel(i);
                
                % Compute total thrust
                FthrustDecel(i) = FlimpropDecel(i);
            else 
                FlimpropDecel(i) = - Flimthrust(xdotDecel(i-1));

                % Compute total drag
                Fdrag = Fdrag_aeroDecel(i) + Fdrag_hoverDecel(i) + Fdrag_brakeDecel(i);
                Fdrag = Fdrag - FlimpropDecel(i);     % Auxiliary drag using LIM
...
%     end
    ...
    Flimprop = [Flimprop, FlimpropDecel];
...
% %     plot(x,v_override)
    ...
    fprintf(['Trajectory case no. ' num2str(caseno) ': ' num2str(mpod) 'kg pod | ' num2str(P/1000,4) 'kPa | ' num2str(T,4) 'K | ' num2str(x1,4) 'm @' num2str(gForce_pusher,2) 'g push | ' num2str(xdot1,4) 'm/s max velocity | ' num2str(deltat_cruising,2) 's cruising | ' num2str(deltat_LIM,2) 's LIM propulsion | ' num2str(brakegapNom) 'mm final nominal brakegap'])
%     fprintf(['Trajectory case no. ' num2str(caseno) ': ' num2str(mpod) 'kg pod | ' num2str(P/1000,4) 'kPa | ' num2str(T,4) 'K | ' num2str(x1,4) 'm @' num2str(gForce_pusher,2) 'g push | ' num2str(xdot1,4) 'm/s max velocity | ' num2str(deltat_cruising,2) 's cruising | ' num2str(gForce_brakedrag,2) 'g brake drag | ' num2str(brakegapNom) 'mm final nominal brakegap'])
    fprintf('\n')
    ...
    axis([0 1.25*t(length(t)) 0 xf*1.1])
    grid on
    grid minor    
%     title(['Trajectory case no. ' num2str(caseno) ': ' num2str(mpod) 'kg pod | ' num2str(P/1000,4) 'kPa | ' num2str(T,4) 'K | ' num2str(x1,4) 'm @' num2str(gForce_pusher,2) 'g push | ' num2str(xdot1,4) 'm/s max velocity | ' num2str(deltat_cruising,2) 's cruising | ' num2str(gForce_brakedrag,2) 'g brake drag | ' num2str(brakegapNom) 'mm final nominal brakegap'])
    ...
    subplot(412)
    plot(t,xdot)
    axis([0 1.25*t(length(t)) 0 1.2*120])
    grid on
    grid minor
    ylabel('Velocity (m/s)')
...
%     axis([0 1.25*t(length(t)) -1.5 1.5])
    axis([0 1.25*t(length(t)) -2.2 2.2])
    grid on
    grid minor
